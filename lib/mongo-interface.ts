...
export default class MongoInterface  {
  dbo: any;
   ...
   async createCollectionUniqueIndexes()
    ...
    async createIndexOnCollection(collectionName:string, indexColumnName:string)
    {
      this.dbo.collection(collectionName).createIndex( { [`${indexColumnName}`]: 1 }, { unique: false } )
    }

    async createUniqueIndexOnCollection(collectionName:string, indexColumnName:string)
    {
      this.dbo.collection(collectionName).createIndex( { [`${indexColumnName}`]: 1 }, { unique: true } )
    }

    async createDualIndexOnCollection(collectionName:string, indexColumnNameA:string, indexColumnNameB:string)
    {
      this.dbo.collection(collectionName).createIndex( { [`${indexColumnNameA}`]: 1,  [`${indexColumnNameB}`]: 1 }, { unique: false } )
    }


    async createUniqueDualIndexOnCollection(collectionName:string, indexColumnNameA:string, indexColumnNameB:string)
    {
    ...
    async createUniqueSparseDualIndexOnCollection(collectionName:string, indexColumnNameA:string, indexColumnNameB:string)
    {
    ...
    async insertOne(collectionName:string,obj:any)
    {
    //  var myobj = { name: "Company Inc", address: "Highway 37" };
      return new Promise((resolve, reject) => {
          this.dbo.collection(collectionName).insertOne(obj, function(err: any, res: any) {
            if (err) reject(err);
    ...
    async dropCollection(collectionName:string)
    {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.dbo.collection(collectionName).drop( function(err: any, res: any) {
           if (err) reject(err);
           resolve(res);
         });
